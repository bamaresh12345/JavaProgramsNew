package ZRestAssuredProject.FromFilesSchemaValidateBodyRequest;
...
public class SchemaValidation {
    ...
    public String putAPIRequestBody = FileUtils.readFileToString(new File(FileNameConstants.JSON_PUT_REQUEST_BODY),"UTF-8");
     ...
     String token=""; //data Chaining for token  *********** IMP

    int bookingId =0;  //data Chaining for bookingId  *********** IMP
    ...
    public void JsutGetBookingIDfromPost() throws IOException {
        ...
        Assert.assertEquals(resp.getStatusCode(), 200);  //*********** IMP

        bookingId = resp.path("bookingid"); // will get the latest booking ID  *********** IMP
    ...
    @Test
        public void SchamaValidationfromFile()
        {
           Response resp = (Response) RestAssured
                    ...
                    .assertThat()
                    .statusCode(200)
                    .body(JsonSchemaValidator.matchesJsonSchema(jsonSchema));  // need json-schema-validator  dependency  *********** IMP
    ...
    @Test
    public void GenerateTokenPostMethod()
    {
        //token generation
        ...
        token = responsetoken.path("token"); // --> this very good use path and id  *********** IMP
